...
class TestClass(object):
    ...
    def test_add_remove_multiple(self):
        ...
        my_fleet = []
        for i in range(my_queue.get_variables()['max_queue_depth']):
            my_fleet.append(Car(queue_id=my_queue.get_variables()['queue_id']))
        my_queue.add_cars(my_fleet)
    ...
    def test_is_full(self, full_queue):
        my_queue = full_queue
        assert my_queue.is_full() is True

    def test_notify(self, full_queue):
        my_queue = full_queue
        assert my_queue.get_variables()['state'] == 'waiting'
        my_queue_len = len(my_queue)

        no_car_returned_yet = True
        while no_car_returned_yet:
            notify_results = my_queue.notify(can_move=True)
            if notify_results[2] is None:
                assert my_queue[0].get_variables()['distance_moved'] < 2
            else:
                assert type(notify_results[2]) is Car
                no_car_returned_yet = False

        assert len(my_queue) == my_queue_len - 1
